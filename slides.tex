...
\onslide<2->{\begin{equation}\begin{tikzcd}[ampersand replacement=\&]
...
\onslide<3->{\hil{Aber:} Äquivariante Kohomologie $H_G^*(M) = H^*(EG\times_G M)$
    mit der Borel Konstruktion ist kein gutes Ziel für den Chern-Charakter!}
...
\begin{frame}{Der Delokalisierte Chern-Charakter}
    \begin{block}{Delokalisierte Kohomologie [Baum, Connes]}
...
\only<2->{\hil{Frage:} Wie kann man eine solche Erweiterung konkret
konstruieren?}
\end{frame}
...
\begin{frame}{$K$-Theorie über ein Spektrum}
    \begin{exampleblock}{Theorem [Atiyah]}{}
        ...
        geeignete Unterräume in der Normtopologie mit Konjugationswirkung.
    \end{exampleblock}
    ...
    entlang von Abbildungen zurückziehen. (\glqq Indexbündel\grqq)}
\end{frame}
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item einem Differenzialform-Repräsentanten ihres Chern-Charakters
            $\Ch(x)$.
    \end{itemize}
    \onslide<2->{Wenn wir einen universellen Repräsentanten für den
        Chern-Charakter finden, können wir also möglicherweise differenzielle
        $K$-Theorie
    klassifizieren.
    ...
    $\textcolor{blue}{\Rightarrow}$ Wir brauchen also besser geeignete Modelle
    der klassifizierenden Räume.}
...
\begin{frame}{Klassifizierende Räume für $K$-Theorie}
    ...
    \begin{align*}
        ...
        U^1 &\to \mathrm U^1
    \end{align*}
    entspricht dabei der Summe von Untervektorräumen bzw. Blocksumme von
    Matrizen.
\end{frame}
\begin{frame}
...
\begin{frame}{Äquivariante Differenzielle $K$-Theorie}
    \begin{block}{Definition/Theorem}
        ...
        \begin{align*}
            ...
            \Omega^1_{\mathrm{delok}}(M)} \color{black}{/\sim} \;\;\mathrm{und}\\
...
\end{frame}
\begin{frame}{Zykel-Abbildungen (gerader Fall)}
    ...
    \end{align*}
    die wir die \hil{topologische Zykel-Abbildung} nennen.

    Ein \hil{geometrischer Lift} der topologischen Zykel-Abbildung ist eine
...
\end{frame}
\begin{frame}
    Zykel-Abbildungen sind nützlich, um Klassen in $\hat K_G$ explizit zu
    beschreiben.
    \begin{block}<2->{Theorem (Existenz von Zykel-Abbildungen)}
            Die oben definierte differenzielle Erweiterung $\hat K_G^*$ besitzt
            sowohl eine gerade als auch eine ungerade geometrische Zykel-Abbildung.
    \end{block}
    \begin{block}<3->{Theorem (Äquivariante Eindeutigkeit)}
        Bis auf Isomorphie ist $\hat K_G^*$ die eindeutige differenzielle
        Erweiterung, die sowohl eine gerade, als auch eine ungerade
        Zykel-Abbildung zulässt. 
    \end{block}
\end{frame}
