...
\begin{frame}{$K$-Theorie über ein Spektrum}
    ...
    \end{exampleblock}
    \onslide<2->{\textcolor{blue}{Idee:} Die universellen Räume $\mathscr{F}_i$
        enthalten jeweils eine \emph{universelle} $K$-Theorie Klasse, welche wir
    entlang Abbildungen zurückziehen. (``Indexbündel'')}
\end{frame}
\begin{frame}
...
\begin{frame}
    \begin{block}{Vermutung (Äquivariantes Venice-Lemma)}
        ...
        Formen die Chern--Simons Form einer $G$-Homotopie 
        \begin{align*}
            f\colon I\times M \to
            \mathrm{Gr}_{\mathrm{res}}\quad \mathrm{oder}\quad f\colon I\times M
            \to\mathrm U^1,
        \end{align*}
        wobei $f_0$ die konstante Abbildung auf den Basispunkt ist.
    \end{block}
    Wenn das äquivariante Venice-Lemma wahr ist, so kann man stets auf Zykel
    $(f,\omega)$ mit $\omega=0$ reduzieren. Somit ist in diesem Fall jede Klasse
    in $\hat K_G$ durch eine Abbildung $f$ charakterisiert.
\end{frame}
\begin{frame}{Zykelabbildungen (gerader Fall)}
    ...
    \onslide<2->{Ein \emph{geometrischer Zykel} ist ein Paar
    $(E,\nabla)$ bestehend aus 
    \begin{itemize}
        \item einem $G$-Vektorbündel $E\to M$.
        \item einem invarianten Zusammenhang $\nabla$ auf $E$.
\end{itemize}}
\end{frame}
\begin{frame}{Zykelabbildungen (gerader Fall)}
    ...
    \begin{itemize}
        ...
        \item<2-> die Verknüpfung $I\circ \widehat{\mathrm{cycl}} =
            \mathrm{cycl}$.
    \end{itemize}
    \onslide<2->{Im ungeraden Fall gibt es (über den Suspensionsisomorphismus)
    eine ungerade topologische Zykelabbildung.}
\end{frame}
\begin{frame}
